...
只有**你按下按鈕**時才會重新載入頁面 —— 那一次請求是你發動的，跟自己按 F5 一樣。
頁首的「全部更新（含 truney）」與 truney 卡片上的「重新整理並讀取」都算，
兩顆都會重載。**排程永遠不會自己重載 truney。**

規則是「排程不得自動重載」，不是「永遠不重載」。
...
| 指標 | 來源 | 性質 |
|---|---|---|
| S&P 500 | multpl.com | 市場公認的公開序列，每交易日更新 |
| 0050 | **自行計算** | 見下 |
| 台股全市場 | **自行計算** | 同一套算法套用到全部上市個股 |

三者並列的理由：0050 的本益比是集中於半導體的估值（台積電權重約五成），
不代表台股整體。要回答「台股 vs 美股哪個貴」，該比的是**台股全市場 vs
S&P 500** —— 兩者都是全市場、都用總量法，層級才對等。0050 那格回答的是
另一個問題：「我持有的標的貴不貴」。

兩者都**沒有公開的歷史序列**可抓，走勢從第一次執行當天開始累積。

### 0050 的本益比是算出來的，不是抓來的

證交所的 `BWIBBU_ALL` 只涵蓋 1,082 檔**個股**，不含 ETF；元大官網是 SPA，
原始 HTML 內沒有數字。所以 `scripts/tw_index_pe.py` 用官方資料自行計算：

1. TWSE OpenAPI 取三份：每日收盤價、已發行普通股數、個股本益比
2. 市值 = 收盤價 × 已發行普通股數，取前 50 大近似 0050 成分股
3. **整體本益比 = Σ市值 ÷ Σ盈餘**，個股盈餘 = 市值 ÷ 本益比

第 3 步是指數本益比的標準定義（總量法），不是把 50 個本益比平均 ——
平均會被高本益比個股拉高，總量法才反映「買下整個組合要幾年盈餘回本」。

已知誤差，dashboard 上有標注：

- 成分股用**總市值**排名近似，真正的台灣50指數採自由流通量調整，且每季才調整成分股
- 虧損公司證交所顯示「-」，排除在分母外；`coverage` 顯示納入計算的市值佔比
